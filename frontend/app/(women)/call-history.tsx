...
import { subscribeToCreatorCallHistory, CallRecord as FirestoreCall, getAvatarSource, fetchMoreCreatorCallHistory, formatFirebaseDate } from '../../services/firebaseService';

const CallItem = memo(({ item, colors, isMissed }: { item: FirestoreCall, colors: any, isMissed: boolean }) => (
  <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
    <View style={styles.imageContainer}>
      <Image 
        source={getAvatarSource(item.callerAvatar, 'man')} 
        style={styles.image} 
        contentFit="cover"
        transition={200}
      />
    ...
    <View style={styles.info}>
      <View style={styles.topRow}>
        <Text style={[styles.name, { color: colors.text }]}>{item.callerName || 'Anonymous'}</Text>
...
export default function WomenCallHistory() {
  ...
  const fetchInitialData = useCallback(async () => {
    ...
    try {
      const response = await fetchMoreCreatorCallHistory(user.uid, null, 20);
  ...
  const loadMore = async () => {
    ...
    try {
      const response = await fetchMoreCreatorCallHistory(user.uid, lastDocRef.current, 20);
      ...
      console.error("Error loading more creator call history:", error);
    } finally {
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            ...
            <Text style={[styles.headerSub, { color: colors.subText }]}>Recent conversations with your Buddies</Text>
          </View>
        }
        refreshControl={
          <RefreshControl
